module DocflowsHelper
  ...
  def docflow_new_button
    if User.current.admin? || User.current.edit_docflows? || User.current.edit_docflows_in_some_category?
  ...
  def docflow_delete_button
    ...
    if User.current.admin? ||
      ( @doc.versions.size == 1 && @doc.last_version.status.id < 3 && (User.current.edit_docflows? || User.current.edit_docflows_in_category?(@doc.docflow_category_id)) )
  ...
  def docflow_edit_button
    @doc = @version.docflow if @doc.nil?
    if User.current.admin? ||
      ( @version.docflow.last_version.status.id  < 4 && (User.current.edit_docflows? || User.current.edit_docflows_in_category?(@doc.docflow_category_id)) )
