module DocflowsHelper
  ...
  def docflow_new_button
    if authorized_globaly_to?(:docflows, :new) || User.current.edit_docflows_in_some_category?
  ...
  def docflow_delete_button
    ...
    if @doc.versions.size == 1 && @doc.last_version.status.id < 3 && (authorized_globaly_to?(:docflows, :destroy) || User.current.edit_docflows_in_category?(@doc.docflow_category_id))
  ...
  def docflow_edit_button
    @doc = @version.docflow if @doc.nil?
    if @version.docflow.last_version.status.id  < 4 && (authorized_globaly_to?(:docflows, :edit) || User.current.edit_docflows_in_category?(@doc.docflow_category_id))
